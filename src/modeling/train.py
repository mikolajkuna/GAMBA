...
from src.config import (
    ...
    MCMC_CHAINS, MCMC_DRAWS, MCMC_TUNE, RANDOM_SEED, RAW_DATA  
)
...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Keep only records with non-negative values above the minimum wage
...
# =====================================================
def train_bayesian_gam_pymc(synthetic_path: str,
                              n_samples=MCMC_DRAWS,   # from config
                              n_tune=MCMC_TUNE,       # from config
                              n_chains=MCMC_CHAINS):  # from config
    ...
    with pm.Model() as model:
        ...
        beta_gender_child = pm.Normal("beta_gender_child", mu=0, sigma=500)
        ...
        mu = (intercept +
              ...
              beta_gender_child * X_scaled[:, child_idx] * X[:, gender_idx])
    ...
    with model:
        posterior_pred = pm.sample_posterior_predictive(
            trace, progressbar=True, random_seed=RANDOM_SEED  
        )
    ...
    beta_gender_child_samples = trace.posterior['beta_gender_child'].values.reshape(-1)
    ...
    for i in range(n_mcmc_samples):
        pred_m_all[i, :] = (
            ...
            beta_gender_child_samples[i] * X_cf_m[:, child_idx] * 1
        )
        pred_f_all[i, :] = (
            intercept_samples[i] +
            ...
            beta_gender_child_samples[i] * X_cf_f[:, child_idx] * 0
        )
    ...
    if beta_gender_child_posterior.mean() > 0:
        print(f"\n  • Men with children earn MORE (+{beta_gender_child_posterior.mean():.0f} PLN per child)")
    else:
        print(f"\n  • Men with children earn LESS ({beta_gender_child_posterior.mean():.0f} PLN per child)")

    print("\n" + "="*60)
    print("Gender Pay Gap by Number of Children:")
    print("="*60)
    for n_child in sorted(np.unique(X[:, child_idx])):
        mask = X[:, child_idx] == n_child
        if mask.sum() < MIN_GROUP_SIZE:
            continue
        print(f"  {int(n_child)} children: mean gap = {gap_adjusted[mask].mean():,.0f} PLN (n={mask.sum()})")
...
def train_all_gams(synthetic_path: str, include_bayesian=False):
    ...
    if include_bayesian and 'bayesian' in results:
        bm = results['bayesian']['model']
        print(f"Bayesian (PyMC)  - Mean Gap: {bayes_gap.mean():,.0f} PLN")
        print(f"                   94% HDI: [{bm['gap_hdi'][0]:,.0f}, {bm['gap_hdi'][1]:,.0f}] PLN")
        print(f"                   P(gap > 0): {bm['gap_prob_positive']:.1%}")
...
if __name__ == "__main__":
    print(f"Using data: {RAW_DATA}")
    print(f"Random seed: {RANDOM_SEED}")
    results = train_all_gams(str(RAW_DATA), include_bayesian=True)
