...
def train_gam_reml(X, y, weights=None):
    gam = LinearGAM(
        s(0, lam=10, constraints="monotonic_inc") +  # age
        f(1) +                                       # gender
        s(2, lam=10, constraints="monotonic_inc") + # education_level
        s(3, lam=10, constraints="monotonic_inc") + # job_level
        s(4, lam=10, constraints="monotonic_inc") + # experience_years
        f(5) +                                       # distance_from_home
        s(6, lam=10, constraints="monotonic_dec") + # absence
        s(7, lam=10) +                               # child
        te(1, 3, lam=10)                             # interaction gender x job_level
    )
...
def train_gam_bayes(X, y, prior="weak"):
    ...
    with pm.Model() as model:
        ...
        beta = pm.Normal(
            "beta", 
            mu=0, 
            sigma=2000 if prior=="informative" else 10000, 
            shape=n_features
        )
